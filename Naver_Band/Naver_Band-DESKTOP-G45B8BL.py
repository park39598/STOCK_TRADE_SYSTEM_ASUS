...
class Naver_Band():
        ...
        for i, content in enumerate(content_df['spell_spacing_correction_filter']):
            for screen in self.HTH_CMD.HTH_CMD['스크리닝']:
                if screen in content:
                    screen_list.append(content_df.iloc[i].name)
                    break
        result_df = content_df.drop(screen_list, axis=0)
        return result_df
    ...
    def HTH_Comment_Meme_Filter(self, content_df):
        ...
        content_df.columns = ['content_screen_filter']
        ...
        for i, content in enumerate(content_df['content_screen_filter']):
            cnt = 0
    ...
    def post_comment_analysis_algo(self, data):
        ...
        screen_sort_df = self.HTH_Comment_Screen_Filter(spell_spacing_correction_df)

        data = pd.merge(data, screen_sort_df, on='created_at', how='left')
        data.to_excel('d:\\screen_sort_df_total.xlsx')
        ...
        data.to_excel('d:\\Meme_Filter_df_total.xlsx')
        self.logging.logger.debug("HTH_Comment_Meme_Filter_END!")

        buy_name_sort, sell_name_sort, sell_buy_name_sort, ambiguous_sort = self.jongmok_name_sort(buy, sell, sell_buy, ambiguous)
        ...
        data = pd.merge(data, temp_df, on='created_at', how='left')
        data.to_excel('d:\\jongmok_name_sort_df_total.xlsx')
        self.logging.logger.debug("jongmok_name_sort_END!")

        buy_price_ratio_sort = self.buy_price_ratio_sort(buy_name_sort)
        self.logging.logger.debug("buy_price_ratio_sort_END!")
        sell_price_ratio_sort = self.sell_price_ratio_sort(sell_name_sort)
        self.logging.logger.debug("sell_price_ratio_sort_END!")
        sell_buy_price_ratio_sort = self.sell_buy_price_ratio_sort(sell_buy_name_sort)
...
if __name__ == "__main__":
    ...
    a = band._All_post_comment_Read()
    print(a.head())
    a.to_excel(os.path.join(os.getcwd(),"test.xlsx"))
    # print(b)
    app.exec_()
    # print(b)
