...
class Kiwoom(QAxWidget):
    ...
    def __init__(self, data_queue=None, order_queue=None):
        ...
        for condition in PORT_PARAMETER.WantConditionList:
            if (condition in PORT_PARAMETER.MorningList) and (now < 931):
                ...
                pass
        # 실시간 Screen 관련
        # 실시간종목 screen 시 Thema 체크를 위한 데이터 로드
        self.Thema = Crolling_ThemaList_Infostock()
        self.Thema_Profit_df = None
        df = self.Thema.Make_Thema_Period_Profit()
        del self.Thema
        temp_thema_df = pd.DataFrame(data=None, columns=['0주전','1주전', '2주전', '3주전', '4주전', '5일', '10일', '15일', '20일', '25일'])
        for i, period in enumerate(tqdm.tqdm(temp_thema_df.columns)):
        ...
        for period in temp_thema_df.columns:
            for thema in temp_thema_df[period]:
                stock_df = self.DB.DB_LOAD_Table("stocks_thema_list", thema)
                temp_dict[period].update({thema: list(stock_df.replace("a", "").index)})
        self.Thema_Screen_df = temp_dict
    # Schdule 모듈
    ...
    def Final_Value_Betting(self):
        for condition in PORT_PARAMETER.종가베팅_박범진:
            self.Set_Condition_Receive(condition, 1)
    ...
    def _receive_real_data(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            self.Port_Que_Dict[port_name].put([sCode, self.account_stock_dict, self.portfolio_stock_dict[port_name], self.jango_dict, self.not_account_stock_dict, self.use_money, self.account_num, self.finance_screen_df, self.Thema_Screen_df])
