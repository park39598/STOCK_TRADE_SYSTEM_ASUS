...
from Backtest import asset_corr
import Kiwoom.Kiwoom
from Backtest.config.log_class import Logging
...
class BACK_TEST:
    def __init__(self):
        ...
        self.SQL = self.fs.SQL
        self.pr = Price_data()
        self.quant = Quant_Strategy()
        self.asset_correlation = asset_corr.ASSET_CORR_TEST()
        self.DB_system_parameter = 'system_parameter'
        self.CodebyName_df = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.fs.TOTAL_JONGMOK_NAME_TABLE)
        ...
        self.MAX_ASSET_RATIO = 0.55
        self.MIN_ASSET_RATIO = 0.4
        self.manual_stock_list = []



    def back_test_asset(self, price_df, strategy_df, start_date, end_date, initial_money, money_ratio, Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio, tax_on=True):
        # 전략의 상위종목들 DF의 종목코드를 추려내기
        # 백테스트의 인덱스만 코드로 들어오면 데이터 형태는 무관...
        ...
        if Asset_strategy_off :
            asset_money = 0
            stock_money = (initial_money - cash_amount- asset_money)
        else:
            asset_price = Asset_price_df[Asset_strategy_ratio.index].loc[start_date:end_date].astype('float').dropna()
            asset_money = (initial_money - cash_amount) * asset_ratio
            stock_money = (initial_money - cash_amount - asset_money)
            for asset in Asset_strategy_ratio.index:
                each_amount = int(asset_money) * float(Asset_strategy_ratio.loc[asset])
                temp = int(each_amount / float(asset_price[asset][0]))
                pf_asset_num[asset] = temp
                if tax_on:
                    asset_amount = asset_amount + temp * asset_price[asset][0] * (
                                1 - self.trans_tax)  # 구입시 거래세+수수료 0.33%
                else:
                    asset_amount = asset_amount + temp * int(asset_price[asset][0])
                asset_pf = asset_pf + asset_price[asset].astype('int') * pf_asset_num[asset] * (1 - self.trans_tax)
                jongmok_status_df.loc[asset] = 0
                jongmok_status_df['START금액'][asset] = int(asset_price[asset][0])
                jongmok_status_df['END금액'][asset] = int(asset_price[asset][-1])
                jongmok_status_df['수량'][asset] = int(pf_asset_num[asset])
                jongmok_status_df['손익'][asset] = (int(asset_price[asset][-1]) - int(asset_price[asset][0])) * \
                                                 pf_asset_num[
                                                     asset]
                jongmok_status_df['수익률'][asset] = ((float(asset_price[asset][-1]) / float(
                    asset_price[asset][0])) - 1) * 100

                jongmok_status_df['종목명'][asset] = asset
        try:
            each_money = stock_money / len(strategy_df.index)
        except:
            each_money = 0
        ...
        if tax_on:
            cash_amount = initial_money - asset_amount - stock_amount* (1 - self.trans_tax)# 구입시 거래세+수수료 0.33%
            backtest_df = pd.DataFrame({'주식포트폴리오': stock_pf * (1 - self.trans_tax),
                                        'ASSET포트폴리오':asset_pf})
        else:
            cash_amount = initial_money - asset_amount - stock_amount
        ...
        if Asset_strategy_off :
            ...
            for asset_month in asset_pf.index:
                temp = backtest_df['ASSET포트폴리오'].loc[str(asset_month.year) + '-' + str(asset_month.month)]
                temp = temp.fillna(value=temp.iloc[-1])
                backtest_df['ASSET포트폴리오'].loc[str(asset_month.year) + '-' + str(asset_month.month)] = temp
            backtest_df['주식포트폴리오'] = backtest_df['주식포트폴리오'].fillna(method='ffill')
        backtest_df['수수료'] = stock_amount * (self.trans_tax)  #추후 포트폴리오 거래세 확인 후 수정필요
        backtest_df['현금포트폴리오'] = cash_amount
        backtest_df['종합포트폴리오'] = backtest_df['주식포트폴리오'] + backtest_df['ASSET포트폴리오'] + backtest_df['현금포트폴리오']
        ...
        return backtest_df
    def back_test_manual_portfolio(self, price_df, manual_portfolio_df, start_date, end_date, initial_money, money_ratio,
                        Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio, tax_on=True):
        # 전략의 상위종목들 DF의 종목코드를 추려내기
        #        jongmok_status_df = pd.DataFrame(data=strategy_df, index = strategy_df.index , columns =strategy_df.columns + ['START금액','END금액','수량', '손익','수익률'], dtype='float')
        manual_portfolio_df['each_money'] = 0
        jongmok_status_df = manual_portfolio_df.copy()
        jongmok_status_df['START금액'] = 0
        jongmok_status_df['END금액'] = 0
        jongmok_status_df['수량'] = 0
        jongmok_status_df['손익'] = 0
        ...
        jongmok_status_df['손실종목'] = 0
        self.total_trade_num = self.total_trade_num + 1
        strategy_price = price_df[manual_portfolio_df.index][start_date:end_date].astype('float').dropna()
        # cash 계산
        pf_stock_num = {}
        stock_amount = 0
        stock_pf = 0
        ...
        #비중지정에 따른 매수금액 설정
        try:
            for i in range(len(manual_portfolio_df)):
                manual_portfolio_df['each_money'].iloc[i] = stock_money * manual_portfolio_df['비율'].iloc[i].astype('float')
        except:
            each_money = 0
        for num, code in enumerate(strategy_price.columns):
            temp = int(float(manual_portfolio_df['each_money'].loc[code]) / float(strategy_price[code][0]))
            pf_stock_num[code] = temp
            if tax_on:
                stock_amount = stock_amount + temp * strategy_price[code][0]
            else:
            ...
            if jongmok_status_df['수익률'][code] > 0:
                ...
                jongmok_status_df['손실종목'][code] = 1
            jongmok_status_df['종목명'][code] = self.CodebyName_df.loc[code][0]
            if num == 0 : backtest_df = pd.DataFrame(data = strategy_price[[code]],columns=[code])
            else : backtest_df[code] = strategy_price[[code]]
        if tax_on:
            cash_amount = initial_money - asset_amount - stock_amount * (1 - self.trans_tax)  # 구입시 거래세+수수료 0.33%
            backtest_df['주식포트폴리오'] = stock_pf * (1 - self.trans_tax)
            backtest_df['ASSET포트폴리오'] = asset_pf
            #backtest_df = pd.DataFrame({'주식포트폴리오': stock_pf * (1 - self.trans_tax), 'ASSET포트폴리오': asset_pf})
        else:
            cash_amount = initial_money - asset_amount - stock_amount
            backtest_df['주식포트폴리오'] = stock_pf
            backtest_df['ASSET포트폴리오'] = asset_pf
            #backtest_df = pd.DataFrame({'주식포트폴리오': stock_pf, 'ASSET포트폴리오': asset_pf})
        self.per_profitSum = self.per_profitSum + jongmok_status_df[jongmok_status_df['수익률'] > 0]['수익률'].sum()
        self.per_lossSum = self.per_lossSum + jongmok_status_df[jongmok_status_df['수익률'] <= 0]['수익률'].sum()
        if Asset_strategy_off:
            pass
            ...
            for asset_month in asset_pf.index:
                ...
                temp = temp.fillna(value=temp.iloc[-1])
                backtest_df['ASSET포트폴리오'].loc[str(asset_month.year) + '-' + str(asset_month.month)] = temp
                # asset자산을 이용하지 않을경우... OFF를 해줘야하는데 하지않을경우 ratio만 0.0으로 될경우 종합포트폴링오에 N/A가 들어가는 경우가 있어서 아래 코드 삽입
                if asset_ratio == 0.0: backtest_df['ASSET포트폴리오'].loc[
                    str(asset_month.year) + '-' + str(asset_month.month)] = 0
            # 백테스트 자료(ASSET)가 month단위로 밖에 없어서 빈slot을 ASSET자산자료의 달 Value로 바꿔주자
            backtest_df['주식포트폴리오'] = backtest_df['주식포트폴리오'].fillna(method='ffill')
        backtest_df['수수료'] = stock_amount * (self.trans_tax)  # 추후 포트폴리오 거래세 확인 후 수정필요
        backtest_df['현금포트폴리오'] = cash_amount
        backtest_df['종합포트폴리오'] = backtest_df['주식포트폴리오'] + backtest_df['ASSET포트폴리오'] + backtest_df['현금포트폴리오']
        ...
        # backtest_log make
        try:
            if manual_portfolio_df.columns[1] == '종목':
                self.get_backtest_log(start_date, end_date, manual_portfolio_df.index[0], None, jongmok_status_df, True)
            else:
                self.get_backtest_log(start_date, end_date, manual_portfolio_df.columns[0], manual_portfolio_df.columns[2],
                                      jongmok_status_df, True)
        except:
            self.get_backtest_log(start_date, end_date, manual_portfolio_df.columns[0], "_NONE", jongmok_status_df, True)
        return backtest_df

    def make_manual_portfolio(self, stock_list, ratio_list):
        code_list = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.fs.TOTAL_JONGMOK_NAME_TABLE)
        manual_portfolio_df = pd.DataFrame(data=stock_list, columns=['code'])        
        manual_portfolio_df['종목'] = 0
        manual_portfolio_df = manual_portfolio_df.set_index('code')
        for code in manual_portfolio_df.index:
            manual_portfolio_df['종목'].loc[code] = code_list.loc[code][1]
        manual_portfolio_df['비율'] = ratio_list
        return manual_portfolio_df
    ...
    def select_code_by_price_excel(self, price_df, data_df, start_date, end_date, index_date, comp_size,
        ...
        try:  # 시가총액이 없는 에러때문에
            ...
            pass
        if BJ_Filter:
            select_invest_df = select_invest_df[(select_invest_df['부채비율(%)'].astype('float') < 200) & (
                        select_invest_df['유보율(%)'].astype('float') > 300) & (
    ...
    def rebal_peri_stochastics(self, start_date, end_date, comp_type):
        if comp_type == '소형주':
            rebal_date = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.pr.KOSDAQ_PRICE_TABLE)
        else:
            rebal_date = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.pr.KOSPI_PRICE_TABLE)
    ...
    def get_mdd_cagr(self, back_test_df, start_date, end_date):
        ...
        for i in back_test_df.index[1:]:
            ...
            if max_list[-1] > max_list[-2]:
                ...
                mdd_list.append(min((back_test_df['총변화율'][i] - max_list[-1]) / (1+max_list[-1]), mdd_list[-1]))
                #mdd_list.append(min(temp, mdd_list[-1]))
        back_test_df['max'] = max_list
        back_test_df['MDD'] = mdd_list
        total_month = (int(end_date.split('-')[0]) - int(start_date.split('-')[0]))*12 + int(end_date.split('-')[1]) - int(start_date.split('-')[1])
        num_of_year = int((float(total_month)/12) + 0.5)
        #num_of_year = int(end_date.split('-')[0]) - int(start_date.split('-')[0]) + 1
    ...
    def make_invest_from_quarter(self, index_date, start_date):
        ...
        expect_profit_df = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.fs.EXPECT_PROFIT_TABLE)
        ...
        '''
        if year_month[1] == '12':
            RIM_ROE = self.SQL.DB_LOAD_Table('stocks_finance_year', index_date, True)
            ...
            RIM_ROE = self.SQL.DB_LOAD_Table('stocks_finance_year', quarter_RIMROE_index, True)
    ...
    def rebal_backtest_excel(self, start_date, end_date, initial_money, strategy_sel, rebal_P, asset_strategy, asset_ratio, comp_size='TOTAL',
                             rim_on=True, comp_del=True, BJ_Filter=True, money_ratio=0.0, money_ratio_method='buy_increase',
                             tax_on=True, portpolio=None, portfolio_list=None, portfolio_ratio =None):
        # 리밸런싱 결과 변수 초기화
        self.plus_jongmok = 0
        self.minus_jongmok = 0
        self.total_trade_num = 0
        ...
        self.per_lossSum = 0
        # ASSET_DATA_LOAD
        if asset_strategy == None:
            Asset_strategy_off = True
            Asset_strategy_ratio = None
            Asset_price_df = None
        else:
            Asset_strategy_off = False
            Asset_strategy_df = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.quant.portfolio_stratey_table)
            Asset_price_df = self.SQL.DB_LOAD_Table(self.DB_system_parameter, self.quant.portfolio_price_table)
            Asset_price_df.index = pd.to_datetime(Asset_price_df.index)
            Asset_strategy_ratio = Asset_strategy_df.loc[asset_strategy].T.dropna().T
            Asset_strategy_ratio = Asset_strategy_ratio.iloc[1:]
        ...
        for i in range(len(Rebalance_selections)):
            ...
            if strategy_sel == '저PER':
                ...
                low_per = self.quant.get_value_rank_quarter(invest_df, price_df, start, end, strategy_date, 'PER', None,
                                                            stock_num).dropna()
                temp_df = self.back_test_asset(price_df, low_per, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                low_pbr = self.quant.get_value_rank_quarter(invest_df, price_df, start, end, strategy_date, 'PBR', None,
                                                            stock_num).dropna()
                temp_df = self.back_test_asset(price_df, low_pbr, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
            elif strategy_sel == 'F스코어':
                fs_in_df = self.make_invest_from_quarter(strategy_date, start)
                # price_in_df = self.pr.DB_LOAD_Price_Data()
                (fs_df, price_df) = self.select_code_by_price_excel(price_in_df, invest_in_df, start, end,
                                                                    strategy_date, comp_size, rim_on, comp_del,
                                                                    BJ_Filter, 2)
                fs_score = self.quant.get_fscore(fs_df, price_df, strategy_date, stock_num, start, 'past')
                temp_df = self.back_test_asset(price_df, fs_score, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                momentum = self.quant.get_momentum_rank(momentum_price, self.price_df[start_date].index[0], 3,
                                                        stock_num).dropna()
                temp_df = self.back_test_asset(price_df, momentum, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                temp_df = self.back_test_asset(price_df, magic, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                super_value = self.quant.super_value_strategy_by_kang(invest_df, price_df, start, end, strategy_date,
                                                          stock_num).dropna()
                # magic.to_excel('f:\\magic.xlsx')
                temp_df = self.back_test_asset(price_df, super_value, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                temp_df = self.back_test_asset(price_df, fixed_df, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                temp_df = self.back_test_asset(price_df, week_high_52, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                ...
                for code in code_list.index:
                    pass_success, price_data_latest = self.Trade_algo.Average_Break_Strategy(code, 120, self.logging, real_trade=False, start = start)
                    if pass_success == True:
                        total['code'].append(code)
                        total['종목명'].append(code_list.loc[code][1])
                total_df = pd.DataFrame(data = total)
                total_df = total_df.set_index('code')
                temp_df = self.back_test_asset(price_df, total_df, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
            elif strategy_sel == 'MANUAL_PORTFOLIO':
                #한국형 올웨더
                self.manual_stock_list = portfolio_list
                ratio_list = portfolio_ratio
                #self.manual_stock_list = ['A214430']
                #ratio_list = [1.0]
                #비율이 1.0이 되면 백테스트 시작
                if int(sum(ratio_list)+0.00000001) == 1.0:
                    manual_portfolio_df = self.make_manual_portfolio(self.manual_stock_list , ratio_list)
                    temp_df = self.back_test_manual_portfolio(price_in_df, manual_portfolio_df, start, end, initial_money, money_ratio,
                                               Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, asset_ratio,
                                               tax_on)
                else:
                    print("비중을 다시 조절하세요~!!!!!!")
                ...
                if KOSPI_flag == False:
                    ...
                    temp_df = self.back_test_asset(price_in_df, KOSPI, start_date, end_date, initial_money, 0.0,
                                                   Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, tax_on)
                ...
                if KOSDAQ_flag == False:
                    ...
                    temp_df = self.back_test_asset(price_in_df, KOSDAQ, start_date, end_date, initial_money, 0.0,
                                                   Asset_strategy_off, Asset_strategy_ratio, Asset_price_df, tax_on)
                    KOSDAQ_flag = True
                else:
                    pass
...
if __name__ == "__main__":
    ...
    REF_rebal_P = 12
    # 리밸런스 백테스트
    (test.KOSPI, result_KOSPI) = test.rebal_backtest_excel(start_date, end_date, initial_money, 'KOSPI', REF_rebal_P, asset_ratio=0.5,
                                                           asset_strategy = None, BJ_Filter=True, comp_size='TOTAL',
                                                           rim_on=True, comp_del=False, money_ratio=0.0, tax_on=True,
                                                           portpolio=None)

    (test.KOSDAQ, result_KOSPI) = test.rebal_backtest_excel(start_date, end_date, initial_money, 'KOSDAQ', REF_rebal_P, asset_ratio=0.5,
                                                            asset_strategy = None, BJ_Filter=True, comp_size='TOTAL',
                                                            rim_on=True, comp_del=False, money_ratio=0.0, tax_on=True,
                                                            portpolio=None)
    ...
    rebal_P1 = 12
    strategy1 = '슈퍼가치전략'
    portfolio_list = ['A252670', 'A214430', 'A003470', 'A005930']
    portfolio_ratio = [0.3, 0.4, 0.2, 0.1]
    compsize1='소형주'
    portpolio1 = None
    BJ_Filter1 = False
    rim_on1 = False
    comp_del1= False
    asset_strategy1 = None
    (TEST_DF1, RESULT_DF1) = test.rebal_backtest_excel(start_date, end_date, initial_money, strategy1, rebal_P1, asset_ratio=0.0,
                                                       asset_strategy=asset_strategy1, BJ_Filter=BJ_Filter1, comp_size=compsize1,
                                                       rim_on=rim_on1, comp_del=comp_del1, money_ratio=0.0,
                                                       money_ratio_method='buy_increase', tax_on=True, portpolio=portpolio1,
                                                       portfolio_list= portfolio_list, portfolio_ratio= portfolio_ratio)

    RESULT_DF1.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\RESULT_DF1.xlsx')
    TEST_DF1_TAG = strategy1 + compsize1 + str(BJ_Filter1) + str(rim_on1) + str(comp_del1) + str(portpolio1) + str(asset_strategy1) + '_'+ str(rebal_P1)
    ...
    rim_on2 = False
    comp_del2 = False
    asset_strategy2 = None
    (TEST_DF2, RESULT_DF2) = test.rebal_backtest_excel(start_date, end_date, initial_money, strategy2, rebal_P2,asset_ratio=0.0,
                                                       asset_strategy=asset_strategy2, BJ_Filter=BJ_Filter2, comp_size=compsize2,
    ...
    TEST_DF2_TAG = strategy2 + compsize2 + str(BJ_Filter2) + str(rim_on2) + str(comp_del2) + str(portpolio2) + str(asset_strategy2) + '_'+ str(rebal_P2)
    ...
    rim_on3 = False
    comp_del3 = False
    asset_strategy3 = None
    (TEST_DF3, RESULT_DF3) = test.rebal_backtest_excel(start_date, end_date, initial_money, strategy3, rebal_P3,asset_ratio=0.0,
                                                       asset_strategy=asset_strategy3, BJ_Filter=BJ_Filter3, comp_size=compsize3,
    ...
    TEST_DF3_TAG = strategy3 + compsize3 + str(BJ_Filter3) + str(rim_on3) + str(comp_del3) + str(portpolio3) + str(asset_strategy3) + '_'+ str(rebal_P3)
    ...
    rim_on4 = False
    comp_del4 = False
    asset_strategy4 = None

    (TEST_DF4, RESULT_DF4) = test.rebal_backtest_excel(start_date, end_date, initial_money, strategy4, rebal_P4, asset_ratio=0.0,
                                                       asset_strategy=asset_strategy4, BJ_Filter=BJ_Filter4, comp_size=compsize4,
    ...
    TEST_DF4_TAG = strategy4 + compsize4 + str(BJ_Filter4) + str(rim_on4) + str(comp_del4) + str(portpolio4) + str(asset_strategy4) + '_'+ str(rebal_P4)
    ...
    test.KOSPI.to_excel('F:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\REF_KOSPI.xlsx')
    test.KOSDAQ.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\REF_KOSDAQ.xlsx')
    TEST_DF1.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\TEST_DF1.xlsx')
    TEST_DF2.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\TEST_DF2.xlsx')
    TEST_DF3.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\TEST_DF3.xlsx')
    TEST_DF4.to_excel('f:\\OneDrive - Office 365\\EXCEL\\BACKTEST_Quarter_RESULT\\TEST_DF4.xlsx')
    ...
    plt.legend()
    plt.subplot(2, 1, 2)
    test.KOSPI['MDD'].plot(label='KOSPI' + ' = ' + format(test.KOSPI['MDD'].min() * 100, "3.2f") + '%')
    test.KOSDAQ['MDD'].plot(label='KOSDAQ' + ' = ' + format(test.KOSDAQ['MDD'].min() * 100, "3.2f") + '%')
    TEST_DF1['MDD'].plot(label=TEST_DF1_TAG + ' = ' + format(TEST_DF1['MDD'].min() * 100, "3.2f") + '%')
    TEST_DF2['MDD'].plot(label=TEST_DF2_TAG + ' = ' + format(TEST_DF2['MDD'].min() * 100, "3.2f") + '%')
    TEST_DF3['MDD'].plot(label=TEST_DF3_TAG + ' = ' + format(TEST_DF3['MDD'].min() * 100, "3.2f") + '%')
    TEST_DF4['MDD'].plot(label=TEST_DF4_TAG + ' = ' + format(TEST_DF4['MDD'].min() * 100, "3.2f") + '%')
    plt.legend()
    ...
    # corr
    test.asset_correlation.korea_ETF_test(test.manual_stock_list, TEST_DF1.index[0], TEST_DF1.index[-1])
    plt.show()
